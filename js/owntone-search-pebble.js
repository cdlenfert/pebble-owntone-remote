...
function performSearch(type, query) {
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === 4) {
      sendDictionaryToPebble({[KEY_STATUS]:'HTTP_DONE:' + xhr.status, [KEY_DEBUG]: (xhr.responseText || '')});
      if (xhr.status !== 200) {
        console.log('Search HTTP error: ' + xhr.status);
...
Pebble.addEventListener('appmessage', function(e) {
  ...
  if (payload[KEY_CMD] == 1) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        ...
        try {
          var resp = JSON.parse(xhr.responseText);
          var items = [];
          if (resp.playlists && resp.playlists.items) items = resp.playlists.items;
          else if (resp.artists && resp.artists.items) items = resp.artists.items;
          else if (resp.albums && resp.albums.items) items = resp.albums.items;
          var dict = {};
          dict[KEY_RESULT_COUNT] = items.length;
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        sendDictionaryToPebble({[KEY_STATUS]:'HTTP_DONE:' + xhr.status});
        if (xhr.status === 200 || xhr.status === 204) sendDictionaryToPebble({[KEY_STATUS]:'OK'});
        else sendDictionaryToPebble({[KEY_STATUS]:'Error: HTTP ' + xhr.status});
      }
    };
